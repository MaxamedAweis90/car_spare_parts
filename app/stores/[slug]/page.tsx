...
import { buildProductImageUrl, findProductsBySellerId } from "@/lib/server/productService";
...
function buildStoreAssetUrl(fileId: string | null, kind: "avatar" | "banner") {
  ...
  const avatarBucket = process.env.NEXT_PUBLIC_APPWRITE_STORE_AVATAR_BUCKET_ID || process.env.APPWRITE_STORE_AVATAR_BUCKET_ID;
  const bannerBucket =
...
export default async function StorePage({ params }: StorePageProps) {
  ...
  const heroFallback = "https://images.unsplash.com/photo-1549921296-3b4a698c73e1?auto=format&fit=crop&w=1600&q=80";
  const bannerImage = bannerUrl ?? heroFallback;
  const description = store.storeDescription || "From first sip to the last mile, we keep your ride running at its best.";
    ...
    <div className="bg-white min-h-screen">
      <div className="mx-auto w-full max-w-10/12 px-4 sm:px-6 lg:px-8">
        <section className="overflow-hidden rounded-b-[32px]">
            ...
            <div className="relative flex h-full items-end px-6 pb-6">
              <div className="flex items-center gap-4 text-white">
                <div className="flex h-24 w-24 items-center justify-center overflow-hidden rounded-2xl border border-white/40 bg-white/20 backdrop-blur">
                  {avatarUrl ? <img src={avatarUrl} alt={`${store.storeName} logo`} className="h-full w-full object-cover" /> : <span className="text-2xl font-semibold">{initials}</span>}
                </div>
                <div className="space-y-2">
                  <h1 className="text-3xl font-bold sm:text-4xl">{store.storeName}</h1>
                  <p className="text-sm text-white/80">Curated by {store.storeSlug}</p>
                </div>
              </div>
            </div>
          </div>
          <div className="flex flex-col gap-4 border-b border-(--color-border) bg-white px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-4">
              <div className="hidden sm:flex h-12 w-12 items-center justify-center overflow-hidden rounded-full border border-(--color-border) bg-white">
                {avatarUrl ? <img src={avatarUrl} alt={`${store.storeName} avatar`} className="h-full w-full object-cover" /> : <span className="text-lg font-semibold text-(--color-text)">{initials}</span>}
              </div>
              <div>
                <p className="text-lg font-semibold text-(--color-text)">{store.storeName}</p>
                <p className="text-xs font-semibold uppercase tracking-[0.25em] text-(--color-muted)">Official storefront</p>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-3 border-b border-(--color-border) bg-white px-6 py-3 sm:flex-row sm:items-center sm:justify-between">
            <nav className="flex flex-wrap items-center gap-4 text-sm font-semibold text-(--color-muted)">
              {(["Home", "New arrivals", "Top picks", "Bundles", "Reviews", "More"] as const).map((item, index) => (
                <span
                  key={item}
                  className={`cursor-pointer pb-1 ${index === 0 ? "border-b-2 border-(--color-primary) text-(--color-text)" : "hover:text-(--color-text)"}`}
                >
                  {item}
                </span>
              ))}
            ...
            <div className="relative w-full max-w-xs">
              <svg aria-hidden viewBox="0 0 16 16" className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 fill-(--color-muted)">
        ...
        <section className="mt-6 rounded-3xl border border-(--color-border-strong) bg-white p-8 text-center shadow-panel">
          <h2 className="text-2xl font-bold text-(--color-text)">{description}</h2>
          <p className="mt-3 text-sm text-(--color-muted)">Quality automotive upgrades handpicked by {store.storeName}. Shop the collections below.</p>
        </section>

        <section className="mt-6 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-xl font-extrabold text-(--color-text)">Featured collections</h3>
            <a href="/products" className="text-sm font-semibold text-(--color-primary) hover:text-(--color-primary-strong)">
              View all
            </a>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            {featuredProducts.length > 0 ? (
              featuredProducts.map((product) => (
                <article key={product.id} className="rounded-3xl border border-(--color-border) bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                  ...
                  <div className="px-4 py-3">
                    <p className="text-sm font-semibold text-(--color-text)">{product.name}</p>
                    {formatPrice(product.price) && (
                      <p className="mt-1 text-sm font-extrabold text-(--color-text)">{formatPrice(product.price)}</p>
                    )}
                  </div>
                </article>
              ))
              ...
              <div className="sm:col-span-2 lg:col-span-5 flex items-center justify-center rounded-3xl border border-dashed border-(--color-border) bg-(--color-surface) px-6 py-12 text-center">
                <div className="space-y-2">
                  <p className="text-sm font-semibold text-(--color-text)">No products showcased yet</p>
                  <p className="text-xs font-medium text-(--color-muted)">Add products to your catalog to populate this featured strip automatically.</p>
                </div>
              </div>
            )}
          </div>
        ...
        <section className="mt-8 grid gap-6 lg:grid-cols-[2fr,1fr]">
          <div className="space-y-4 rounded-3xl border border-(--color-border) bg-(--color-surface) p-6 shadow-sm">
            <h3 className="text-lg font-extrabold text-(--color-text)">Store highlights</h3>
            <ul className="space-y-3 text-sm text-(--color-text)">
              <li className="flex items-start gap-2">
                <svg aria-hidden viewBox="0 0 16 16" className="mt-1 h-3.5 w-3.5 fill-(--color-primary)">
          ...
          <div className="space-y-4 rounded-3xl border border-(--color-border) bg-white p-6 shadow-sm">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-(--color-muted)">Contact email</p>
              <p className="mt-2 text-sm font-semibold text-(--color-text)">{store.contactEmail || "Not provided"}</p>
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-(--color-muted)">Contact phone</p>
              <p className="mt-2 text-sm font-semibold text-(--color-text)">{store.contactPhone || "Not provided"}</p>
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-(--color-muted)">Last updated</p>
              <p className="mt-2 text-sm font-semibold text-(--color-text)">{new Date(store.updatedAt).toLocaleString()}</p>
            </div>
          </div>
        </section>
      </div>
