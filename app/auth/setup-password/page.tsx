...
function SetupPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/reset-password", {
        ...
        body: JSON.stringify({ userId, secret, password, name }),
      });
      ...
      if (!res.ok) {
        ...
        setMessage("Account setup successfully! Redirecting to login...");
        setTimeout(() => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      >
        {loading ? "Setting setup..." : "Complete Setup"}
      </button>
    </form>
  );
}
      ...
      <div className="w-full max-w-xl rounded-3xl bg-white p-8 shadow-[0_30px_80px_rgba(15,23,42,0.08)]">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-slate-600 mt-1">
            Welcome! Please set a password for your new administrator account.
          </p>
        </div>
