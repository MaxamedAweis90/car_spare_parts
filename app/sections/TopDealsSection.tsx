...
export function TopDealsSection({ deals, loading, viewMoreHref }: TopDealsSectionProps) {
  const scrollerRef = useRef<HTMLDivElement>(null);

  const formatter = useMemo(() => new Intl.NumberFormat(undefined, { style: "currency", currency: "USD", maximumFractionDigits: 0 }), []);
  ...
  return (
    <section className="space-y-4 rounded-2xl border border-(--color-border-strong) bg-(--color-surface) p-6 shadow-panel">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-[13px] font-semibold uppercase tracking-[0.28em] text-(--color-muted)">Top Deals</p>
          <h2 className="text-2xl font-extrabold text-(--color-text)">Score the lowest prices today</h2>
          <p className="text-sm text-(--color-muted)">Shop limited time drops curated for fast-moving sellers.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Link
            href={viewMoreHref}
            className="inline-flex items-center gap-2 rounded-full bg-(--color-primary) px-4 py-2 text-sm font-semibold text-black shadow-sm transition hover:bg-(--color-primary-strong)"
          >
            View more
            <span aria-hidden>›</span>
          </Link>
      ...
      {loading && <p className="text-sm text-(--color-muted)">Loading top deals...</p>}
      {!loading && deals.length === 0 && <p className="text-sm text-(--color-muted)">No deals available yet. Check back soon.</p>}
      ...
      >
        {deals.map((deal, index) => {
          ...
          const displayCompare = comparePrice ? formatter.format(comparePrice) : null;
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-1 flex-col gap-2 rounded-b-2xl bg-white p-4">
                <h3 className="line-clamp-2 text-sm font-semibold text-(--color-text)">{deal.name}</h3>
                <div className="flex items-baseline gap-2">
                  <span className="text-lg font-bold text-(--color-accent)">{displayPrice}</span>
                  {displayCompare && (
                    <span className="text-xs text-(--color-muted) line-through">{displayCompare}</span>
                  )}
                </div>
                {moqLabel && <p className="text-xs font-medium text-(--color-muted)">{moqLabel}</p>}
              </div>
            </article>
          );
        })}
