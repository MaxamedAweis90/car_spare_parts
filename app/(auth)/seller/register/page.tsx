...
function SellerRegisterContent() {
  ...
  // Avoid flashing the form while resolving session or when redirecting.
  if (shouldHide) {
    return null;
  }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Redirect to pending page with context.
      const params = new URLSearchParams();
      if (email) params.set("email", email);
      router.push(`/auth/seller/pending${params.toString() ? `?${params}` : ""}`);
    } catch (error) {
      console.error(error);
      ...
      <div className="w-full max-w-5xl overflow-hidden rounded-3xl bg-white shadow-[0_30px_80px_rgba(15,23,42,0.12)] md:grid md:grid-cols-2">
        <div className="px-8 py-10 sm:px-12 flex flex-col gap-6">
          <div className="text-left">
            ...
            <h1 className="mt-4 text-3xl font-extrabold text-slate-900">Apply to become a seller</h1>
            <p className="text-sm text-slate-600">Admin approval is required. We&apos;ll notify you when you&apos;re approved.</p>
          </div>
        ...
        <div className="relative hidden md:block bg-[#1f4fe0]">
          ...
          <div className="relative flex h-full flex-col items-center justify-center gap-6 px-8 py-12 text-white">
            <div className="rounded-full bg-white/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em]">Grow your store</div>
            <h2 className="text-2xl font-extrabold">Reach more buyers with us.</h2>
            <p className="max-w-sm text-sm text-white/85">List products, manage orders, and get insights to boost sales.</p>
