...
export default function LoginPage() {
  ...
  // Avoid flashing the page while we resolve the session, or after we know we must redirect.
  if (shouldHide) {
    return null;
  }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Customer portal only: any non-customer is treated as invalid credentials.
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(59,130,246,0.08),transparent_35%),radial-gradient(circle_at_bottom_right,rgba(59,130,246,0.08),transparent_35%)] bg-[#f7f9fc] flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-xl rounded-3xl bg-white p-8 shadow-[0_30px_80px_rgba(15,23,42,0.08)]">
        ...
        {oauthError && <div className="mb-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">OAuth failed. Try again.</div>}
        {message && <div className="mb-3 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-800">{message}</div>}
        ...
        <div className="grid gap-3 sm:grid-cols-1">
          ...
          >
            <img src="https://cdnjs.cloudflare.com/ajax/libs/simple-icons/13.16.0/google.svg" alt="Google" className="h-5 w-5" />
            Log in with Google
          </button>
        </div>
